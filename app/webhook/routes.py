...
def handle_voice_control(event, user, text):
    """處理語音控制按鈕"""
    try:
        # 解析語音控制指令
        parts = text.split('_')
        if len(parts) >= 3:
            control_type = parts[1]  # slow, normal, fast, replay
            ...
            rate = rate_map.get(control_type, 1.0)
            
            # 發送指定語速的語音檔
            from ..api.helpers import send_voice_with_rate
            send_voice_with_rate(user.line_user_id, order_id, rate, user.preferred_lang)
            
        else:
            # 無效的語音控制指令
            error_messages = {
                "en": "Invalid voice control command.",
                "ja": "無効な音声制御コマンドです。",
                "ko": "잘못된 음성 제어 명령입니다.",
                "zh": "無效的語音控制指令。"
            }
        ...
        print(f"處理語音控制失敗：{e}")
        error_messages = {
            "en": "Failed to process voice control. Please try again.",
            "ja": "音声制御の処理に失敗しました。もう一度お試しください。",
            "ko": "음성 제어 처리에 실패했습니다. 다시 시도해 주세요.",
            "zh": "語音控制處理失敗，請重試。"
        }
        message = error_messages.get(user.preferred_lang, error_messages["zh"])
...
def handle_temp_voice_control(event, user, text):
    """處理臨時訂單語音控制按鈕"""
    try:
        # 解析臨時訂單語音控制指令
        parts = text.split('_')
        if len(parts) >= 4:
            control_type = parts[2]  # slow, normal, fast, replay
            processing_id = int(parts[3])
            ...
            # 取得處理記錄
            from ..models import GeminiProcessing
            processing = GeminiProcessing.query.get(processing_id)
            
            if processing and processing.structured_menu:
                # 重建臨時訂單
                import json
                menu_data = json.loads(processing.structured_menu)
                
                # 這裡需要從前端傳來的訂單資料重建臨時訂單
                # 暫時發送確認訊息
                confirm_messages = {
                    "en": f"Generating voice file with {rate}x speed...",
                    "ja": f"{rate}x の速度で音声ファイルを生成中...",
                    "ko": f"{rate}x 속도로 음성 파일을 생성 중...",
                    "zh": f"正在生成 {rate}x 語速的語音檔..."
                }
                ...
                error_messages = {
                    ...
                    "ja": "注文が見つかりません。新しい注文を作成してください。",
                    "ko": "주문을 찾을 수 없습니다. 새 주문을 생성해 주세요.",
                    "zh": "找不到訂單，請重新建立訂單。"
                }
                message = error_messages.get(user.preferred_lang, error_messages["zh"])
